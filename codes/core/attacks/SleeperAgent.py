...
class SleeperAgent(Base):
    ...
    def _train_model(self, model, log, trainset, testset, poison_sourceset, poison_testset, augment, device, schedule, is_save_best_model):
        ...
        # opt = torch.optim.Adam([{'params':params_1x}, {'params': model.fc.parameters(), 'lr': lr*10}], lr=lr, weight_decay=weight_decay)
        ...
        for epoch in range(1,epochs+1,1):
            ...
            if is_save_best_model is True:
                if train_acc > best_acc:
                    best_acc = train_acc
                    save_path = os.path.join(self.work_dir, "best_model.pth")
                    self.best_model= model
                    torch.save(model.state_dict(), save_path)
                    log(f"保存best_model acc:{best_acc} to {save_path}")
            # test
            model.eval()
            with torch.no_grad():
                # on benign testset
                if testset is not None:
                    predict_digits, labels, loss = self._test(testset, device, batch_size, model)
                ...
                if poison_sourceset is not None:
                    predict_digits, labels, loss = self._test(poison_sourceset, device, batch_size, model)
                ...
                if poison_testset is not None:
                    predict_digits, labels, loss = self._test(poison_testset, device, batch_size, model)
                    full_asr = (predict_digits.max(1)[1]==labels).sum().item() * 100. / len(labels)
                else:
                    full_asr = 0

            msg = f"Epoch: {epoch}/{epochs}, train_acc: {round(train_acc,2)}, " + \
            f"benign_test_acc: {round(test_acc,2)}, source_asr: {round(source_asr,2)}, full_asr: {round(full_asr,2)}"
    ...
    def craft_poison_dataset(self, model, log, init_model, trainset, testset, craft_iters, retrain_iter_interval, y_target, y_source, poison_num, augment, batch_size, num_workers, retrain_schedule, eps, device, test_schedule):
        ...
        print("patch_source_trainset[0][0] mean:",patch_source_trainset[0][0].mean().item())
        ...
        source_grad_loader = torch.utils.data.DataLoader(patch_source_trainset, batch_size=source_batch_size, 
        ...
        scheduler = torch.optim.lr_scheduler.MultiStepLR(att_optimizer, milestones=[craft_iters // 2.667,
                                                                                    craft_iters // 1.6,
                                                                                    craft_iters // 1.142],)
        dataloader = torch.utils.data.DataLoader(poison_set, batch_size=batch_size, 
                                                 #num_workers=num_workers, 
        ...
        for t in range(1, craft_iters+1):
            ...
            with torch.no_grad():
                ...
                # poison_imgs.to(torch.device("cpu"))
                # poison_deltas.to(torch.device("cpu"))        
                poison_deltas.data = torch.clamp(poison_deltas, min=-eps, max=eps)
                poison_deltas.data = torch.max(torch.min(poison_deltas,1-poison_imgs), -poison_imgs)
                # poison_imgs.to(device)
                # poison_deltas.to(device)
            ...
            if t % retrain_iter_interval == 0 and t!= craft_iters: 
                ...
                predict_digits, labels, loss = self._test(source_testset, device, batch_size, model)
                source_test_acc = (predict_digits.max(1)[1]==labels).sum().item() * 100. / len(labels)
                
                # test on poisoned testset(source class only)
                predict_digits, labels, loss = self._test(patch_source_testset, device, batch_size, model)
                source_test_asr = (predict_digits.max(1)[1]==labels).sum().item() * 100. / len(labels)
                
                # test on benign testset(all classes)
                predict_digits, labels, loss = self._test(self.test_dataset, device, batch_size, model)
                test_acc = (predict_digits.max(1)[1]==labels).sum().item() * 100. / len(labels)

                # test on poisoned testset(all classes)
                predict_digits, labels, loss = self._test(full_patch_testset, device, batch_size, model)
        ...
        return poison_trainset, patch_source_testset, full_patch_testset, poison_ids, poison_deltas
    ...
    def train(self, init_model, schedule=None):
        ...
        #     self.model.load_state_dict(torch.load(self.current_schedule['pretrain'])["state_dict"], strict=True)
        ...
        if self.current_schedule['benign_training'] is True:
            ...
            self._train_model(self.model, log, self.train_dataset, self.test_dataset, None, None ,augment, device, self.current_schedule, is_save_best_model = True)
        elif self.current_schedule['benign_training'] is False:
            # log("开始准备trigger数据集:patch_source_testset, patch_testset")
            ...
            # craft poison dataset 
            # poison_trainset, patch_source_testset, full_patch_testset, poison_ids, poison_deltas
            log("开始准备poisoned数据集:self.poisoned_train_dataset")
            # 因为不想改变self.model（pretrained in imageNet）
            
            self.poisoned_train_dataset, self.patch_source_testset, self.full_patch_testset, self.poison_ids, self.poison_deltas = \
            ...
            self._train_model(
                ...
                testset=self.test_dataset, # benign testset
                poison_sourceset=self.patch_source_testset, # trigger set
                poison_testset=self.full_patch_testset, # trigger set
                augment=augment, 
                device=device, 
