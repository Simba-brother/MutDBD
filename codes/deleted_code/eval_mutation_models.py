...
# # from codes.ourMethod.target_class import TargetClassProcessor
...
# # def eval_mutated_model(model_struct, poisoned_trainset, mutation_operator_name):
#     '''
#     dataset/model_name/attack_name/mutation operator
#     评估在整个后门训练集(poisoned trainset)上各个分类上的report
#     Args:
#         model_struct: 变异模型的模型结构
#         poisoned_trainset: 污染的训练集
#         mutation_operator_name: 变异算子的名称
#     Return:
#         report_data:{mutation_rate:report}
...
#                 map_location="cpu")
#             model_struct.load_state_dict(state_dict)
#             evalModel = EvalModel(model_struct, poisoned_trainset, device)
...
# # def sort_mutated_model(model_struct, target_class_clean_set, target_class_poisoned_set):
# #     '''
# #     对变异模型进行排序
# #     Args:
# #         model_struct:变异模型结构
# #         target_class_clean_set: 攻击类别中的干净集,用于选择clean seed
# #         target_class_poisoned_set: 攻击类别中的污染集
# #     Return:
# #         priority_list:[(priority, weight_file),...,]
# #     '''
# #     # 优先级队列q,值越小优先级越高
...
#     # proctitle = dataset_name+"_"+model_name+"_"+attack_name+"_eval_mutated_models"
...
#     start_time = time.perf_counter()
#     proctitle = dataset_name+"_"+model_name+"_"+attack_name+"_sort_mutated_models"
#     setproctitle.setproctitle(proctitle)
#     print(f"proctitle:{proctitle}")
#     sort_mutated_model()
#     end_time = time.perf_counter()
