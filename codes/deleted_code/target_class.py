...
def calu_p_value_and_dela_value(list_target, list_source_list):
    p_value_list = []
    cliff_delta_list = []
    for list_source in list_source_list:
        p_value = stats.wilcoxon(list_target, list_source).pvalue
        p_value_list.append(p_value)
        list_target_sorted = sorted(list_target) # 原来list不改变
        list_source_sorted = sorted(list_source)
        d,info = cliffs_delta(list_target_sorted, list_source_sorted)
        cliff_delta_list.append(abs(d))
    return p_value_list,cliff_delta_list
...
def detect_target_class(
    ...
    '''
    # 满足条件1，即当满足该target_class_i上的均值和中位值都是最大的时，condition_1_flag = True
    condition_1_flag = False
    # 满足条件2，即当满足该target_class_i上的精度分布相较于其他class_i上的精度分布p_value都小于0.01且delat_value都大于0.147时，condition_2_flag = True
    condition_2_flag = False
    max_mean_value = -1
    max_median_value = -1
    max_mean_class_i = -1
    max_median_class_i = -1
    target_class_i = -1
    adaptive_rate  = -1
    for mutate_rate, mutator_name_dict in dict_eval_report.items():
        # 当前变异率下所有变异算子变异模型的eval report list
        report_list = []
        for mutator_name in mutator_name_dict.keys():
            report_list.extend(dict_eval_report[mutate_rate][mutator_name])
        # 当前变异率下，各个类别上的精度
        ...
        for report in report_list:
            ...
            for class_idx in range(class_num):
                ...
                class_dict[class_idx].append(precision)
        for class_i, precision_list in class_dict.items():
            mean_value = np.mean(precision_list)
            median_value = np.median(precision_list)
            if mean_value > max_mean_value:
                max_mean_value = mean_value
                max_mean_class_i = class_i
            if median_value > max_median_value:
                max_median_value = median_value
                max_median_class_i = class_i
        if max_mean_class_i == max_median_class_i:
            condition_1_flag = True
            target_class_i = max_mean_class_i
            target_precision_list = class_dict[target_class_i]
            source_precison_list_list = []
            for class_i in class_dict.keys():
                if class_i != target_class_i:
                    source_precison_list_list.append(class_dict[class_i])
            p_value_list,delta_value_list = calu_p_value_and_dela_value(target_precision_list, source_precison_list_list)
            all_P_flag = all(p_value < 0.05 for p_value in p_value_list)
            all_C_flag = all(d >= 0.147 for d in delta_value_list)
            if all_P_flag is True and all_C_flag is True:
                condition_2_flag =  True
                adaptive_rate = mutate_rate
                break 
    return target_class_i, adaptive_rate
