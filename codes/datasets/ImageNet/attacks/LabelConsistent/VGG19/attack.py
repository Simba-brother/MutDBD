...
# victim model
victim_model = vgg19(pretrained = True)
# 冻结预训练模型中所有参数的梯度
...
in_features = victim_model.classifier[-1].in_features
victim_model.classifier[-1] = nn.Linear(in_features, num_classes)

#adv_model
adv_model = vgg19(pretrained = True)
# 冻结预训练模型中所有参数的梯度
# for param in adv_model.parameters():
#     param.requires_grad = False
num_classes = 30  # 假设我们要改变分类数量为30
in_features = adv_model.classifier[-1].in_features
adv_model.classifier[-1] = nn.Linear(in_features, num_classes)
