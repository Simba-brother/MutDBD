...
def gen_needed_dataset(model_name:str,poisoned_ids:list):
    ...
    # 在最前面进行投毒
    poisoned_trainset = PoisonedDatasetFolder(trainset,3, poisoned_ids,identity_grid,noise_grid,noise=False,poisoned_transform_index=0,poisoned_target_transform_index=0)
    ...
    poisoned_testset = PoisonedDatasetFolder(testset,3,list(range(len(testset))),identity_grid,noise_grid,noise=False,poisoned_transform_index=0,poisoned_target_transform_index=0)
