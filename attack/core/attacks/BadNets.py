...
class AddDatasetFolderTrigger(AddTrigger):
    ...
    def __call__(self, img):
        ...
        if type(img) == PIL.Image.Image:
            # 图像类型为PIL
            img = F.pil_to_tensor(img)  # 并不会归一化, 但是会被转为CHW
            ...
            # H x W x C
                ...
                img = add_trigger(img)
                img = img.permute(1, 2, 0).numpy() # tensor => ndAarray
            return img
        elif type(img) == torch.Tensor:
            # H x W
            if img.dim() == 2:
...
class PoisonedDatasetFolder(DatasetFolder):
    ...
    def __init__(self,
        ...
        '''
        trigger_path = "core/attacks/3*3_youxiajiao.png.png"
